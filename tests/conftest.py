...
from helpers import APISIX_ADMIN, APISIX_PROXY, ADMIN_KEY, MOCK_PDP, wait_for
...
def services_ready():
    """Block until APISIX Admin API and mock PDP are accepting requests."""
    wait_for(f"{APISIX_ADMIN}/apisix/admin/routes", headers={"X-API-KEY": ADMIN_KEY})
    wait_for(f"{MOCK_PDP}/health")
    # Brief pause so APISIX finishes its own etcd bootstrap
    time.sleep(2)
